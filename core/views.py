...
def home(request):
    test = Testimonial.objects.all().order_by('-pub_date')[:3]
    eve = Event.objects.all().order_by('-date')[:3]
    gal = EventGallery.objects.all().order_by('-pub_date')[:9]
    return render(request, 'home.html', {'test' : test, 'eve' : eve, 'gal' : gal}) 
...
@login_required(login_url='/login')
def all_testimonial(request):
    test = Testimonial.objects.all().order_by('-pub_date')
    if test.exists():
        return render(request, 'testimonial.html', {'test' : test})
    else:
        messages.error(request, 'No Testimonials to display')
        return redirect('/')
